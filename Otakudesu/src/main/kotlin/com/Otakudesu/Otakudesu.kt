...
class Otakudesu : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        "ongoing-anime/page/%d/" to "Anime Ongoing",
        "complete-anime/page/%d/" to "Anime Completed"
    )
	...
	private fun Element.toSearchResult2(): AnimeSearchResponse? {
		val title = selectFirst("h2")?.text()?.trim() ?: return null
		val href = selectFirst("a")?.attr("href") ?: return null
		val posterUrl = selectFirst("img")?.attr("src")?.trim()
		val epNum = selectFirst(".epz")?.ownText()
			?.replace(Regex("\\D"), "")
			?.toIntOrNull()
